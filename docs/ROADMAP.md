...
| I6 | Length-bucketed padding + optional cover traffic | S–M | Flat 256-B pad leaks size buckets (Loopix). | I15 | 🟡 **bucketed padding done**: `padLen` (256·4^k classes — 256/1024/4096/…) now in `ratchet.js` (`frameEncrypt` + `groupSenderEncrypt`) and both index.html inline encrypt paths; wire-compatible since receivers read the explicit length prefix. Cover traffic (fake messages) still open |

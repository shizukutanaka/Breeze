...
1. ~~**クライアント側の配線**~~ — **完了(v3.7)**。この項目は監査時点で既に部分的に
   陳腐化しており、再検証で以下が判明した:
   - `/wipe` → `/api/account/delete` 呼び出し: **実装済みだった**(署名付き)。ただし
     `alias` と `groups` を渡していなかったため、リレーには**エイリアスが永久に居座り、
     全グループの名簿にワイプ済みアカウントの id/pub/name が 30 日残留**していた。
     Worker 側は受け口とその依存関係をコメントで明記していたのに、クライアントが
     履行していなかった。v3.7 で両方を渡すよう修正 + Worker テスト 2 本で固定。
   - グループ UI からの leave/delete: **実装済みだった**(連絡先削除経路が
     `'/group/' + gAction` で動的にパスを組むため、素朴な grep では見えなかった)。
   - `src/crypto/` モジュール統合: 設計方針として**意図的に統合しない**。デプロイ物は
     単一ファイル・依存ゼロであり、`src/crypto/*` は参照実装 + ミラードリフト検査対象
     (CLAUDE.md の表を参照)。「未統合」ではなく「そういう構造」。

   教訓: 「エンドポイントを作った=完了」ではない。呼ぶ側が履行して初めて約束になる。
2. **Durable Objects + WebSocket push**(L)— ポーリング廃止、レート制限の
   isolate 跨ぎ精度向上(ROADMAP C10)。
3. ~~**マルチデバイス同期**~~ — **完了(v3.7、Phase 1)**。Threema Ibex のデバイス
   グループ鍵ではなく「**デバイス = もう 1 つの identity**」方式を採用: prekey・受信箱・
   セッションは既に pubkey 単位で完備なので全て再利用し、新規に必要なのは root 署名付き
   デバイス登記簿と送信側 fan-out のみ。ワイヤ形式不変で旧クライアント完全互換。
   1:1 の双方向配送・自己同期・帰属・編集/削除の伝播・添付まで E2E で証明済み。
   グループの副デバイス送信のみ Phase 2 として残置。
4. **PQXDH ハイブリッド**(M–L)— ML-KEM は WebCrypto 未対応のため WASM が必要 =
   ビルドレス制約と衝突。Workers 側は `crypto.subtle` 拡張待ち。
5. ~~**既読レシートのジッタ/オプトアウト**~~ — **完了(v3.7)**。`/quiet N` で既読送信を
   N 秒保留し、±20% のジッタを加える(NDSS'21 のタイミング非匿名化対策)。オプトアウト
   (完全送信しない)は既存の `hideReadReceipts` が担当。**UX 上の動機と一致した**のが
   決め手: 「開いた瞬間に既読がつく」ことこそが返信義務の発生源であり、プライバシー修正と
   通信疲れ対策が同一の変更になった。

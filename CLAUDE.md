...
| `negotiate.js` | **fully inline**: both `negotiate()` (2-party, `_peerSupportsX3dhV5`) and `negotiateGroup()` (N-party AND rule, `_negotiateGroupCaps`/`_computeGroupV5`, wired into `getGroupSenderKey`'s lazy format-freeze + the kick re-negotiation trigger) and `mergeStickyGroupCaps()` (`_mergeStickyGroupCaps`, applied at every roster write: a known member's `group-v5` cap survives a relay that strips it, so no silent v3 downgrade — but a NEW legacy joiner still holds the group on v3; replaced a group-level "pin v5 once true" that locked legacy joiners out) hand-ported next to the X3DH block. `_capabilities` stays separate (local crypto/idb presence checks, unrelated concept despite the similar name) | ✅ yes (all three) |
...
## Validate
Always run after changes: `./validate.sh` (style/convention gate — must print PASSED; the check count grows as gates land, so read the live score rather than trust a number here). It also gates dead i18n keys (both directions: defined-but-unused AND used-but-undefined), unreachable functions, listeners wired to DOM ids that do not exist (tools/dead-wiring.mjs), platform branches nested inside an incompatible one (tools/unreachable-branch.mjs), mismatched HTML tag nesting (tools/html-balance.mjs — one stray `</div>` silently re-parents every element after it, which no test catches because the elements all still exist), and a reference outside `initMessenger()`'s closure to a name declared only inside it (tools/closure-boundary.mjs — the code still parses and every element it touches already exists, so this is invisible to everything except a bare `ReferenceError` at the exact moment that line runs), and a top-level `let`/`const` that `initMessenger()`'s synchronous prefix touches but that is declared after the boot call (tools/boot-tdz.mjs — temporal dead zone, crashes every page load; shipped three times before this gate existed).

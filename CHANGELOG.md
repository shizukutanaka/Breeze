...
## I6 bucketed padding deployed — plus the plain inbox's two silent-loss paths the sealed queue already fixes (branch devin/1790414474-bucketed-padding-inbox-loss, 2026-09-26)

vitest 842 (+9); `src/crypto/ratchet.js`, `index.html`, `_worker.js`, `tests/ratchet.test.js`, `tests/mirror-drift.test.js`, `tests/worker.test.js`, `docs/ROADMAP.md`, `locales/ja.json`, `CHANGELOG.md`, `_headers`, `tauri/src-tauri/tauri.conf.json` (CSP hash propagation).

**I6 — bucketed message padding (Loopix/SimpleX-style size classes).** A flat `MSG_PAD_BOUNDARY=256` pad still reveals plaintext length to ±256 bytes — enough to fingerprint message sizes end-to-end (docs/IMPROVEMENTS.md I6, Loopix arXiv 1703.00536). New `padLen(n)` in `src/crypto/ratchet.js` pads to the next `256·4^k` bucket (256, 1024, 4096, 16384, …), now used by `frameEncrypt` and `groupSenderEncrypt`; the deployed inline mirrors in index.html (`_encryptForRaw` 1:1, `encryptGroupMsg` group) carry the identical computation as a self-contained loop so the mirror-drift extraction slices stay correct. Wire-compatible in both directions — receivers read the explicit `[len]` prefix and never look at pad size — so bucketed senders and flat-256 senders interoperate. New tests pin the bucket schedule (boundary edges 256/257/1024/1025), the ciphertext-length deltas (d.length = padded+16 tag), a 2000-byte round-trip, and inline-vs-reference bucket parity so a drift back to flat-256 fails red instead of re-leaking silently. `/security` display text updated (EN+JA): "256-byte base, ×4 buckets".

**Plain-inbox silent losses the sealed path already confesses.** The sealed queue gained two durability fixes in earlier sessions — an overflow drop counter surfaced on the next poll ("N messages were lost"), and a post-write read-back that re-appends entries erased by a racing writer's last-write-wins clobber — but `handleMsgSend`/`handleMsgPoll` on `inbox:{to}` got neither, despite having the identical read-modify-write KV shape and `capQueueBytes` eviction. Ported both: sends now accumulate `inbox:{id}:dropped` (same 99999 cap, WEEK TTL) and the poll response returns/clears it once, with the client toasting `toastMsgsDropped` exactly as it already does for sealed drops; and the send path now re-reads `inbox:{to}` and re-appends a clobbered entry (identity = payload ciphertext, not ts), re-bumping ts above the just-read tail so the strictly-increasing poll cursor still holds. New worker tests mirror the sealed ones: overflow reported exactly once, deterministic clobber recovery without evicting the winner, and no spurious re-append on clean writes.

**Account-delete residue sweep.** The deletion handler erased `sealed:{id}:hwm` but not either `:dropped` counter — up to 7 days of "how many messages this account lost" lingered in KV after erasure. Both `inbox:{id}:dropped` and `sealed:{id}:dropped` are now in the delete set.

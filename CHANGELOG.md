## Call path: glare recovery, ICE-disconnect grace, trickle buffering, per-call session ids (branch devin/call-glare-ice, 2026-09-21)

The voice/video path ran a manual offer/answer loop with none of the guards the data-channel path already had (MDN "Perfect Negotiation" patterns). Four real bugs, all fixed in `index.html`:

- **Glare**: `call-offer` was only handled while `_callState==='idle'`, so two peers calling at once dropped each other's offer and BOTH timed out after 60s. Now a deterministic tie-break (lower userId is "polite") makes the polite side roll back silently (`endCall(true)` — no call-end signal, which would tear down the surviving side) and auto-accept the peer's offer. One-sided deployment suffices: a legacy peer's rollback-free client just ignores the collision and completes on the incoming answer.
- **Transient ICE kills calls**: `disconnected`/`failed` both ran `endCall()` immediately — any network blip (WiFi→LTE handoff) ended the call. Per spec `disconnected` is recoverable and `failed` can heal via ICE restart, so both now show 'connecting' under an 8s grace window (`CALL_DISC_GRACE_MS`); the offerer re-offers fresh creds via `restartIce()` + a new `call-offer`, which the answerer services in the re-offer branch of `pollCallSignals`.
- **Trickle ICE raced the answer**: `call-ice` arriving before `setRemoteDescription` threw InvalidStateError into a catch and the candidate was lost. Now buffered (`_callIceBuf`, cap 64) and flushed wherever a remote description lands.
- **Stale-signal poisoning**: the relay redelivers room signals for <30s, so a dead attempt's offer/answer could clobber a live call. Every offer/answer now carries a per-call `cid`; answers to other cids are dropped, glare offers older than the local offer+15s are ignored, and the answerer dedupes re-offers via `_callAnsweredSdp`. The superseded poller's connect-timeout is also bound to its cid so it can't kill the attempt that replaced it, and `pollCallSignals` self-dedupes (the isCall-msg path and `acceptCall` used to run two 1s pollers for the whole call).

Line-gate: index.html had ~0 headroom under validate.sh's 15K cap. The fix rides on a legitimate dedup — `cmdOutput()` already existed ("52x→1x" comment) but 34 hand-rolled `createElement/appendChild/scrollTop` sites never used it; they now do, plus `d.scrollIntoView` for post-await fills and a `plain` arg for `textContent` fills — net −260 lines, back to ~14,990.
...
||||||| parent of fc43afb (fix(calls): glare rollback, ICE-disconnect grace, trickle buffer, per-call cid)
